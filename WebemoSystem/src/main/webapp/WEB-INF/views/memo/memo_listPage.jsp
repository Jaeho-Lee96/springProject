...
<body>
	...
	<table id="checkboxTestTbl" border="1px">
       <caption>메모 리스트 및 체크 박스</caption>
		...
		<c:forEach items="${memoList}" var="list">
		    <tr>
            	...
            	<td>${list.MEMO_FAV}</td>
            	<td>
            	<a href="/webmemo/memoView?memoNum=${list.MEMO_NUM}">${list.MEMO_NAME}</a>
            	</td>
            	<td>${list.USER_ID}</td>
 	...
 	<div>
		<c:if test="${page.prev}">
 			<span>[ <a href="/webmemo/memoListPage?num=${page.startPageNum - 1}">이전</a> ]</span>
		</c:if>
		<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}" var="num">
 			<span>
  				<c:if test="${select != num}">
   					<a href="/webmemo/memoListPage?num=${num}">${num}</a>
  				</c:if>    
		...
		<c:if test="${page.next}">
 			<span>[ <a href="/webmemo/memoListPage?num=${page.endPageNum + 1}">다음</a> ]</span>
		</c:if>
 	</div>
    <div>
    	...
    	<button type="button" name="searchBt">검색</button>
    </div>
</body>
</html>